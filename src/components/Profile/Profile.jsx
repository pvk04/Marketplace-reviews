...
function Profile(){
    ...
    async function swapRole(){
        await state.contractInstance.methods.switchActiveRole().send({from: state.currentAcc});
        let userInfo = await state.contractInstance.methods.users(state.currentAcc).call({from: state.currentAcc});
        await state.contractInstance.methods.addHistory(state.currentAcc, "You swapped role").send({from: state.currentAcc});
    ...
    function renderRoleButtons(){
        if (state.role == 0){
            ...
            return(
                <>
                    <button className={styles.acc_button} onClick={demotionRequest}>Demotion request</button>
                    <button className={styles.acc_button} onClick={swapRole}>Swap role</button>
                </>
                )
        }
            ...
            return(
                <>
                    <button className={styles.acc_button} onClick={swapRole}>Swap role</button>
                    <button className={styles.acc_button} onClick={() => {setNewShopModalActive(true)}}>New shop</button>
                </>
            )
        }
    ...
    return(
        <div>
            <div className={styles.account_info}>
                ...
                <h3>Active role: {state.activeRole == 0 ? "Buyer" : state.activeRole == 1 ? "Seller" : "Admin"}</h3>
                <h3>Balance: {balance}</h3>
                <div>
                    <button className={styles.acc_button} onClick={exit}>Exit</button>
                    {renderRoleButtons()}
                </div>
                
            </div>
            <PromotionModal active={promotionModalActive} setActive={setPromotionModalActive}/>
            <NewShopModal active={newShopModalActive} setActive={setNewShopModalActive}/>
            <FunctionalPanel/>
        </div>
